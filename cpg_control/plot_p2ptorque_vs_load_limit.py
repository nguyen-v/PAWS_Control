...
# Load the new CSV file with applied_load and torque_limit
torque_limit_data = pd.read_csv('LOAD_PAYLOAD_VS_MAX_TORQUE.csv')
applied_loads = torque_limit_data['applied_load'].values
torque_limits = torque_limit_data['torque_limit'].values
...
plt.scatter(applied_loads, torque_limits, color='black', linewidth = 4, marker='x', label='Minimum Torque Before Failure')
