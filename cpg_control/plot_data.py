...
def plot_positions(data, fields_to_plot, boolean_fields, title, y_label, factor = 1):
    ...
    # plt.xlim(data['timestamp'][500], data['timestamp'][650]) # FRONT LEG FAIL
    # plt.xlim(data['timestamp'][350], data['timestamp'][500]) # REAR LEG FAIL
    # plt.xlim(data['timestamp'][250], data['timestamp'][400]) # LOAD FAIL
...
file_path = './AMPLIFY_CUSTOM_0.6NM.csv'
...
plot_positions(data, ['position 1', 'position 3'],  ['foot_contact 1', 'foot_contact 3'], 'Positions and Foot Contacts Over Time', 'Synergy activation [deg]', factor = RAD_TO_DEG)
